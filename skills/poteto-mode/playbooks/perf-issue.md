...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Resolve the `perf-issue` pstack model role. Delegate implementation through the active adapter's **Bounded session** protocol with execution role `implementer` and the configured per-run `model`. Do it yourself if delegation is unavailable.
