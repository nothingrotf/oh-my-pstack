...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Resolve the `perf-issue` pstack model role. On Pi, call `pstack_launch` with execution role `implementer` and the complete brief. The router binds the configured per-run `model`. If the router is unavailable, report the capability gap before direct implementation.
