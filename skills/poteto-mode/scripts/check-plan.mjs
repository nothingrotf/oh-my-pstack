...
const LANES = /(?:parallel|multiple|ten) lanes?.* at the PR head/i;
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.test(live.rest)) fail(live.n, `${pr.title}: Verify, live lacks a host-neutral lane declaration`);
