...
This skill guides you through creating effective Agent Skills for compatible hosts. Skills are markdown files that teach the agent how to perform specific tasks: reviewing PRs using team standards, generating commit messages in a preferred format, querying database schemas, or any specialized workflow.
...
If you need clarification, use the host's structured interaction tool when available:

```
Example structured interaction usage:
- "Where should this skill be stored?" with options like ["Personal (~/.agents/skills/)", "Project (.agents/skills/)"]
- "Should this skill include executable scripts?" with options like ["Yes", "No"]
```

If the structured interaction tool is not available, ask these questions conversationally.
...
**IMPORTANT**: Create personal skills only in `~/.agents/skills/`. Create project skills only in `.agents/skills/`. Do not write into host-managed internal skill directories.
...
If the host provides a structured interaction tool, use it for efficient requirement gathering. Otherwise, ask conversationally.
