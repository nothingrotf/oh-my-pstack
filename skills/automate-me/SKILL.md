...
Mining misses intent that hasn't come up yet. Use the host's structured multi-choice interaction when available; otherwise ask one focused question in normal chat. Lower cognitive load, higher hit rate.
...
Apply the sibling **unslop** skill and the live host's skill-authoring guidelines to every line. Both apply to any agent-read prose, not just skills.
...
- The host's skill-authoring facility, when present: authoring process and writing guidelines; otherwise use the package's existing `SKILL.md` conventions.
